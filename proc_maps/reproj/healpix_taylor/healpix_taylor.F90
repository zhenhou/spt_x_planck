module healpix_taylor
    ...
    subroutine proj_init(nside, theta, phi, proj)
    ...
    subroutine deriv_init(map, proj, deriv, max_order, lmax)
        ...
        allocate(deriv%deriv_maps(0:num_proj-1,0:num_terms-1))
        ...
        allocate(map_dp(0:npix-1))
        allocate(maps_der1(0:npix-1,1:2))
        allocate(maps_der2(0:npix-1,1:3))
        allocate(der2_save(0:npix-1,0:num_der2sav-1))
        ...
        do i=0, max_order, 2
            do j=0, max_order, 2
                ...
                do ipix=0, npix-1
                    map_dp(ipix) = der2_save(ipix, ct_der2)
                enddo

                call map2alm(nside, lmax, lmax, map_dp, alms, zbounds, w8ring)
                call alm2map_der(nside, lmax, lmax, alms, map_dp, maps_der1, maps_der2)
                ...
                if (ct_der2_1 .lt. num_der2sav .and. ct_der2_3 .lt. num_der2sav) then
                    if (der2sav_null(ct_der2_1)) then
                        do ipix=0, npix-1
                            der2_save(ipix, ct_der2_1) = maps_der2(ipix,1)
                        enddo
                    ...
                    if (der2sav_null(ct_der2_3)) then
                        do ipix=0, npix-1
                            der2_save(ipix, ct_der2_3) = maps_der2(ipix,3)
                        enddo
                    ...
                    do j_der=j, j+2
                        ...
                        if (deriv_null(i_term)) then 
                            do i_proj=0, num_proj-1
                                ...
                                if (ct_der .eq. 1) deriv%deriv_maps(i_proj, i_term) = maps_der1(ipix, j_der-j+1)
                                if (ct_der .eq. 2) deriv%deriv_maps(i_proj, i_term) = maps_der2(ipix, j_der-j+1)
                            enddo
    ...
    subroutine deriv_finalize(deriv)
    ...
    subroutine taylor_exp(proj, deriv)
